...
async def run_mission():
    ...
    design = await quantum_skill.invoke({
        "action": "generate_grover",
        "num_qubits": 5,
        "pattern": "10101"
    })
    print(f"Architect: Designed Grover circuit with {design['result']['oracle']['num_qubits']} qubits.")
    ...
    await router_skill.invoke({
        "action": "add_endpoint",
        "endpoint_config": {"name": "LocalSimulator", "url": "http://localhost:8080", "weight": 2.0}
    })
    await router_skill.invoke({
        "action": "add_endpoint",
        "endpoint_config": {"name": "CloudSimulator", "url": "https://quantum.cloud.api", "weight": 1.0}
    })
