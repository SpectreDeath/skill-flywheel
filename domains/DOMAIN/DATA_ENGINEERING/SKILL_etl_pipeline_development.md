...
Design and implement robust ETL (Extract, Transform, Load) pipelines.

## Purpose

Enable efficient data movement and transformation for analytics and reporting.

## Capabilities

1. **Data Extraction**: Extract data from various sources
2. **Data Transformation**: Transform data to target format and structure
3. **Data Loading**: Load transformed data to target systems
4. **Pipeline Monitoring**: Monitor and maintain pipeline health

## Usage Examples

### Basic Usage

"Execute etl pipeline development workflow"

### Advanced Usage

"Run etl pipeline development with custom configuration and validation"

## Input Format

```yaml
etl_pipeline_development_request:
  parameters: object
...
```yaml
etl_pipeline_development_result:
  success: boolean
  artifacts: array
  metrics: object
