...
class WatchdogEventLogger:
    ...
    def log_service_failure(self, failed_services: List[Dict[str, Any]]):
        ...
        logger.warning(f"Service failure detected: {len(failed_services)} services down")
    
    def log_recovery_attempt(self, failed_services: List[Dict[str, Any]], recovery_result: Dict[str, Any]):
    ...
    def log_recovery_success(self, recovered_services: List[str]):
        ...
        self.log_event(event)
        logger.info(f"Recovery successful: {len(recovered_services)} services recovered")
...
class RecoveryManager:
    ...
    def trigger_recovery(self, failed_services: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"Triggering recovery for {len(failed_services)} failed services")
...
"""
            ...
            if result.returncode == 0:
                ...
                logger.error(f"Recovery process failed with return code {result.returncode}")
...
class WatchdogMonitor:
    ...
    def __init__(self):
        ...
        self.idle_threshold_minutes = int(os.environ.get("IDLE_THRESHOLD_MINUTES", "30"))
        self.reaper_interval_minutes = int(os.environ.get("REAPER_INTERVAL_MINUTES", "10"))
        # Configuration paths
        self.docker_compose_file = Path(os.environ.get("DOCKER_COMPOSE_FILE", "../../docker-compose.yml"))
        self.mcp_config_file = Path(os.environ.get("MCP_CONFIG_FILE", "../../data/mcp_config.json"))
        self.registry_file = Path(os.environ.get("SKILL_REGISTRY_PATH", "../../data/skill_registry.json"))
    ...
    async def monitor_services(self):
        ...
        logger.info(f"Monitoring {len(SERVICES)} services every {MONITORING_INTERVAL} seconds")
        ...
        while self.running:
            try:
                ...
                if current_time - last_reaper_check >= (self.reaper_interval_minutes * 60):
                ...
                with open(config_path, 'r') as f:
                    ...
                    for key, svc in config.get("services", {}).items():
                        new_services.append({
                            ...
                            "health_url": svc.get("health_check", f"{svc['url']}/health"),
    ...
    async def handle_service_failures(self, failed_services: List[Dict[str, Any]]):
        ...
        if recovery_result.get("success", False):
            ...
            if not remaining_failures:
                ...
                logger.warning(f"Some services still failing after recovery: {failed_recovery}")
        else:
            self.event_logger.log_recovery_failure(
                failed_services, 
                recovery_result.get("error", "Unknown recovery error")
            )
    ...
    async def check_service_activity_and_idle(self):
        ...
        try:
            ...
            if not activity_result or "service_status" not in activity_result:
                logger.warning("Could not get service activity status from discovery service")
                return
            ...
            if idle_services:
                logger.info(f"Found {len(idle_services)} idle services: {[s['service_name'] for s in idle_services]}")
    ...
    async def _get_service_activity_from_discovery(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            activity_url = f"{discovery_url}/tools/get_service_activity"
    ...
    async def handle_idle_services(self, idle_services: List[Dict[str, Any]]):
        ...
        for service_info in idle_services:
            ...
            logger.info(f"Reaper: Service {service_name} has been idle for {idle_time:.1f} minutes, spinning down")
            ...
            if spin_down_result["success"]:
                ...
                logger.error(f"Reaper: Failed to spin down {service_name}: {spin_down_result['error']}")
                self.event_logger.log_event({
                    "timestamp": datetime.datetime.now().isoformat(),
                    "event_type": "SERVICE_SPIN_DOWN",
                    "service": service_name,
    ...
    async def spin_down_service(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Spin down failed: {str(e)}"
            }

    async def update_service_replicas(self, service_name: str, replicas: int) -> Dict[str, Any]:
