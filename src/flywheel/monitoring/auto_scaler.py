...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("auto_scaler.log"), logging.StreamHandler()],
)
...
class MetricsCollector:
    ...
    async def collect_metrics(self) -> ScalingMetrics | None:
        ...
        try:
            ...
            load_score = self._calculate_load_score(prometheus_metrics, docker_metrics)
            ...
            for container in containers:
                try:
                    ...
                    if "cpu_stats" in stats:
                        ...
                        if system_delta > 0:
                            cpu_usage = (cpu_delta / system_delta) * len(
    ...
    def _calculate_load_score(
        self, prometheus_metrics: Dict, docker_metrics: Dict
    ) -> float:
...
class AutoScaler:
    ...
    async def _make_scaling_decision(self, metrics: ScalingMetrics) -> Dict:
        ...
        target_containers = self._calculate_target_containers(
            current_load, predicted_load, current_containers
        )

        # Check if scaling is needed
        if (
            target_containers > current_containers
            and current_load > self.config["scale_up_threshold"]
        ):
            return {
                "should_scale": True,
                "action": "scale_up",
            ...
            target_containers < current_containers
            and current_load < self.config["scale_down_threshold"]
        ):
            return {
                "should_scale": True,
                "action": "scale_down",
        ...
        return {
            "should_scale": False,
            "reason": f"Load within acceptable range: {current_load:.2f}",
        }

    def _calculate_target_containers(
        self, current_load: float, predicted_load: float, current_count: int
    ) -> int:
        ...
        if target_load > self.config["scale_up_threshold"]:
            ...
            # Scale down
            target_count = max(
                self.config["min_containers"],
                int(current_count * self.config["scale_down_factor"]),
            )
        else:
        ...
        target_count = max(
            self.config["min_containers"],
            min(target_count, self.config["max_containers"]),
        )
    ...
    async def _execute_scaling(self, action: Dict, metrics: ScalingMetrics):
        ...
        try:
            ...
            logger.info(
                f"Executing scaling action: {action['action']} from {current_count} to {target_count} containers"
            )
            ...
            logger.info(
                f"Scaling action completed: {action['action']} to {target_count} containers"
            )
    ...
    async def _scale_down(self, target_count: int, current_count: int):
        ...
        containers = self.docker_client.containers.list(
            filters={"label": "mcp_server=true"}, sort="created"
        )
