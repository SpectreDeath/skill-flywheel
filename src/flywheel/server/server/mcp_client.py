...
class MCPClient:
    ...
    async def initialize(self) -> bool:
        ...
        logger.info(f"Initializing MCP client with Discovery service at {self.discovery_url}")
...
class ReportGenerator:
    ...
    def generate_console_report(health_report: Dict[str, Any]):
        ...
        for domain, stats in health_report["by_domain"].items():
            rate = (stats["healthy"] / stats["total"] * 100) if stats["total"] > 0 else 0
            print(f"{domain:20} | {stats['healthy']:3}/{stats['total']:3} | {rate:5.1f}% | {stats['avg_response_time']:.3f}s")
        print()
    ...
    @staticmethod
    def generate_detailed_report(health_report: Dict[str, Any], filename: str = "mcp_health_report.json"):
...
async def main():
    ...
    print("=" * 50)
    if success_rate >= 90:
        print(f"🎉 Excellent! {healthy}/{total} skills ({success_rate:.1f}%) are healthy")
    elif success_rate >= 75:
        print(f"⚠️  Good, but needs attention. {healthy}/{total} skills ({success_rate:.1f}%) are healthy")
    else:
        print(f"🚨 Critical issues detected. Only {healthy}/{total} skills ({success_rate:.1f}%) are healthy")
