...
class AgentOrchestrator:
    ...
    DISCOVERY_SERVICE_URL = os.environ.get(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    url, json={"skill_name": f"domain-{agent_config.domain}"}
                ) as resp:
                    if resp.status == 200:
                ...
                async with session.post(
                    url, json={"skill_name": agent_config.domain}
                ) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        if isinstance(data, dict) and data.get("internal_endpoint"):
    ...
    async def select_mission_model(
    ...
    def register_agent(self, agent_config: AgentConfig):
        ...
        logger.info(
            f"Registered agent: {agent_config.name} ({agent_config.framework.value})"
        )
        ...
        try:
            ...
            if task_id in self.contexts:
                ...
                context = self.create_context(
                    task_id, task_description, initial_context
                )
            ...
            if len(framework_groups) == 1:
                ...
                result = await self._execute_single_framework(
                    framework, framework_groups[framework], context
                )
            else:
    ...
    def _group_agents_by_framework(
        self, agents: List[str]
    ) -> Dict[AgentFramework, List[str]]:
    ...
    async def _execute_single_framework(
        self, framework: AgentFramework, agents: List[str], context: TaskContext
    ) -> OrchestrationResult:
    ...
    async def _execute_multi_framework(
        self, framework_groups: Dict[AgentFramework, List[str]], context: TaskContext
    ) -> OrchestrationResult:
    ...
    async def orchestrate_cross_domain_mission(
        self,
        mission_id: str,
        mission_objective: str,
        target_domains: List[str],
        agents_per_domain: Dict[str, List[str]],
    ) -> OrchestrationResult:
        """
        ...
        if len(target_domains) < 3:
            logger.warning(
                f"Complexity Info: Cross-domain mission '{mission_id}' spans {len(target_domains)} domains. Target is 3+ domains for complex execution."
            )
        ...
        for domain in target_domains:
            try:
                async with aiohttp.ClientSession() as session:
                    url = f"{self.DISCOVERY_SERVICE_URL}/call/find_domain_for_skill"
                    async with session.post(
                        url, json={"skill_name": f"domain-{domain}"}
                    ) as resp:
                        if resp.status == 200:
                            data = await resp.json()
                            if isinstance(data, dict) and data.get("internal_endpoint"):
        ...
        for domain in target_domains:
            ...
            for agent_name in domain_agents:
                selection = await self.select_mission_model(
                    mission_objective, agent_name
                )
                logger.info(
                    f"Model Selection for '{agent_name}': {selection.get('recommended_model')} ({selection.get('strategy')})"
                )
                if agent_name in self.agents:
                    self.agents[agent_name].model = selection.get(
                        "recommended_model", "gpt-4"
                    )

            if endpoint:
                logger.info(
                    f"Dispatching mission segment to remote domain '{domain}' at endpoint {endpoint}"
                )
            else:
                logger.warning(
                    f"Executing domain '{domain}' locally as endpoint resolution failed/missing."
                )
...
class FrameworkAdapter:
    ...
    async def execute_agents(
        self, agent_configs: List[AgentConfig], context: TaskContext
    ) -> OrchestrationResult:
...
class LangChainAdapter(FrameworkAdapter):
    ...
    async def execute_agents(
        ...
        try:
            logger.info(
                f"Executing {len(agent_configs)} agents with LangChain (Skill-Backed)"
            )
            ...
            for agent_config in agent_configs:
                ...
                if _skill_manager_ref:
                    ...
                    for skill_name in _skill_manager_ref.skills:
                        if skill_name.lower() in context.task_description.lower():
                            logger.info(
                                f"Found matching skill '{skill_name}' for agent task."
                            )
                            try:
                                ...
                                logger.warning(
                                    f"Skill execution failed during orchestration: {skill_err}"
                                )
...
class AgentWorkflowBuilder:
    ...
    def add_step(
        self, step_name: str, agents: List[str], task: str
    ) -> "AgentWorkflowBuilder":
        """Add a workflow step."""
        self.workflow_steps.append({"name": step_name, "agents": agents, "task": task})
        return self
...
async def example_workflow_builder():
    ...
    builder.add_agent(
        ...
        step_name="research_step", agents=["researcher"], task="Research the topic"
    ).add_step(
