...
from flywheel.core.registry_search import search_registry
...
async def discover_skills(
    ctx, query: str, domain: str = None, limit: int = 10, include_details: bool = False
):
    """
    ...
    try:
            ...
            for result in search_results:
                ...
                if skill_path.exists():
                    try:
                        ...
                        logger.warning(
                            f"Could not load full content for {result['name']}: {e}"
                        )
        ...
        return {
            ...
            "domains_available": list(
                {skill.get("domain") for skill in filtered_skills}
            ),
        }
...
async def model_select(
    ...
    if any(
        kw in task_lower for kw in ["code", "python", "rust", "javascript", "refactor"]
    ):
        base_model = "DeepSeek-V2-Coder"
        strategy = "Coding Specialist"
    elif any(
        kw in task_lower for kw in ["reason", "logic", "proof", "complex", "math"]
    ):
        base_model = "o1-mini"
    ...
    try:
        ...
        skill = next(
            (
                ...
                if s["name"].lower().replace("-", "_")
                == skill_id.lower().replace("-", "_")
            ),
            None,
        )
        ...
        if not skill:
            return {
                ...
                "available_skills": [s["name"] for s in filtered_skills[:5]],
            }
        ...
        performance_monitor.record_metric(
            skill_id, MetricType.EXECUTION_TIME, time.time() - start_time
        )
        performance_monitor.record_metric(skill_id, MetricType.SUCCESS_RATE, 1.0)

        log_telemetry(
            skill_id, request, duration=time.time() - start_time, status="success"
        )
        return result
...
async def validate_skill(ctx, skill_id: str, validation_type: str = "all"):
    ...
    try:
        ...
        if validation_type in ["dependencies", "all"]:
            validation_results["results"]["dependencies"] = validate_skill_dependencies(
                skill, filtered_skills
            )

        # Security validation
        if validation_type in ["security", "all"]:
            validation_results["results"]["security"] = validate_skill_security(
                skill_path
            )
...
def validate_skill_dependencies(
    skill: Dict[str, Any], all_skills: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if test_framework == "pytest":
                test_results["results"] = run_pytest_tests(
                    skill_path, temp_dir, test_cases
                )
            elif test_framework == "unittest":
                test_results["results"] = run_unittest_tests(
                    skill_path, temp_dir, test_cases
                )
            else:
...
def run_pytest_tests(
    skill_path: Path, temp_dir: str, test_cases: List[Dict[str, Any]]
) -> Dict[str, Any]:
...
async def benchmark_skill(
    ...
    try:
        ...
        performance_monitor.record_metric(
            skill_id, MetricType.QUALITY_SCORE, avg_quality
        )
...
async def orchestrate_agents(
    ...
    try:
        ...
        if agent_type == "autogen":
            orchestration_results["results"] = await orchestrate_autogen_agents(
                task, agents_config
            )
        elif agent_type == "langchain":
            orchestration_results["results"] = await orchestrate_langchain_agents(
                task, agents_config
            )
        elif agent_type == "langgraph":
            orchestration_results["results"] = await orchestrate_langgraph_agents(
                task, agents_config
            )
        elif agent_type == "crewai":
            orchestration_results["results"] = await orchestrate_crewai_agents(
                task, agents_config
            )
        else:
...
async def get_performance_metrics(
    ...
    try:
        ...
        total_executions = sum(
            ...
            for stats in performance_monitor.skill_stats.values()
        )
        total_time = sum(
            stats["total_time"] for stats in performance_monitor.skill_stats.values()
        )
        avg_execution_time = (
...
def register_existing_skills():
    ...
    if not REGISTRY_FILE.exists():
        logger.error(
            f"Registry file not found at {REGISTRY_FILE}. Run reindex_skills.py first."
        )
        return
    ...
    try:
        ...
        for skill in registry:
            ...
            description = (
                skill["description"]
                or skill["purpose"]
                or f"Execute the {skill_id} skill."
            )
        ...
        logger.info(
            f"Registered {registered_count} existing skills from domains: {', '.join(MCP_DOMAINS) if MCP_DOMAINS else 'All'}"
        )
    except Exception as e:
