...
class AdvancedTelemetryManager:
    ...
    def track_advanced_skill_execution(
        self,
        skill_name: str,
        load_time: float,
        ...
        if skill_name not in self.skill_metrics:
            self.skill_metrics[skill_name] = AdvancedSkillMetrics(
                ...
                if not isinstance(dependencies, Mock)
                else 0,
                memory_usage=0.0,
                success_rate=0.0,
                priority_score=0.0,
        ...
        m.success_rate = (
            m.success_rate * (m.execution_count - 1) + (1 if success else 0)
        ) / m.execution_count
    ...
    def calculate_advanced_priority_score(self, skill_name: str) -> float:
        ...
        score = (
            m.execution_count * 0.4
            + m.success_rate * 0.4
            + (1.0 / max(0.1, m.avg_load_time)) * 0.2
        )
