...
try:
    ...
    ML_AVAILABLE = False
    warnings.warn(
        "ML libraries not available for advanced threat detection.", stacklevel=2
    )
...
class SecurityScanner:
        ...
        return {
            ComplianceFramework.SOC2.value: {
                ...
                "requirements": [
                    {
                        "id": "CC6.1",
                        "description": "Logical and physical access controls are selected and implemented to protect against threats from sources outside the system boundaries.",
                    },
                    {
                        "id": "CC6.2",
                        "description": "Logical and physical access controls are selected and implemented to limit access to system components.",
                    },
                    {
                        "id": "CC6.3",
                        "description": "Personnel with access to the system are provided with guidance as to the importance of protecting the confidentiality and integrity of the information handled by the system.",
                    },
                    {
                        "id": "CC6.4",
                        "description": "The entity selects and implements controls to detect, log, and report on events that are indicators of compromise.",
                    },
                    {
                        "id": "CC6.5",
                        "description": "The entity authorizes, modifies, or removes access to data, software, functions, and other protected information assets based on roles, or similar criteria, consistent with the entity's criteria for authorizing access to the system.",
                    },
                ],
            },
            ComplianceFramework.ISO27001.value: {
                ...
                "requirements": [
                    ...
                    {
                        "id": "A.9.2.1",
                        "description": "User registration and de-registration shall be controlled.",
                    },
                    {
                        "id": "A.9.2.2",
                        "description": "User access provisioning shall be controlled.",
                    },
                    {
                        "id": "A.9.4.1",
                        "description": "Information accessing shall be restricted in accordance with the access control policy.",
                    },
                    {
                        "id": "A.9.4.2",
                    ...
                    {
                        "id": "A.10.1.1",
                        "description": "Cryptographic controls shall be used in accordance with policy.",
                    },
                    {
                        "id": "A.12.4.1",
                        "description": "Events shall be detected and the relevant support processes informed as quickly as possible.",
                    },
                ],
            },
            ComplianceFramework.NIST.value: {
                ...
                "requirements": [
                    {
                        "id": "PR.AC-1",
                        "description": "Identities and credentials are issued, managed, verified, revoked, and audited for authorized users, devices, and other assets.",
                    },
                    {
                        "id": "PR.AC-4",
                    ...
                    {
                        "id": "DE.CM-1",
                        "description": "Network baselines are established and managed.",
                    },
                    {
                        "id": "DE.CM-3",
                        "description": "Personnel awareness programs and activities are established or managed to strengthen the cybersecurity culture.",
                    },
    ...
    def _check_soc2_compliance(self, content: str) -> List[Dict[str, Any]]:
        ...
        if re.search(
            r"logger\.(debug|info)\s*\([^)]*(password|secret|key)",
            content,
            re.IGNORECASE,
        ):
            violations.append(
    ...
    def _check_iso27001_compliance(self, content: str) -> List[Dict[str, Any]]:
        ...
        if re.search(
            r"random\.(random|randint|choice|shuffle)\s*\(", content, re.IGNORECASE
        ):
            violations.append(
                {
                    "framework": "ISO27001",
    ...
    def _calculate_risk_score(
        ...
        for vuln in vulnerabilities:
            ...
            risk_score += (
                severity_weight.get(vuln.get("severity", "medium"), 0.5)
                * confidence_weight
            )
    ...
    def _calculate_confidence(self, match: str, pattern: str) -> float:
        ...
        if match.strip() == pattern.replace(r'["\']', "").replace(
            r'[^"\']+', match.strip()
        ):
            confidence += 0.3
    ...
    def _determine_security_level(
        self, risk_score: float, ml_threat_score: float
    ) -> SecurityLevel:
...
class SecurityMonitor:
    ...
    def get_security_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "high_risk_events": len(
                [e for e in relevant_events if e.get("risk_score", 0) > 0.7]
            ),
            "critical_threats": len(
                [e for e in relevant_events if e.get("ml_threat_score", 0) > 0.8]
            ),
    ...
    def _calculate_vulnerability_trends(
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if test_skill_path.exists():
            ...
            print(
                f"Security scan completed: {scan_result.security_level.value} risk level"
            )
        ...
        global_security_monitor.log_security_event(
            {
                ...
                "ml_threat_score": 0.9,
                "vulnerabilities": [
                    {"type": "test", "description": "test vulnerability"}
                ],
            }
        )
