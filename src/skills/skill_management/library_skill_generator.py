...
Key features:
- Uses inspect.getmembers() to enumerate public API surface
- Filters to public callables (functions, classes, and builtin routines)
- Supports focus parameter to limit wrapped components
- Verifies generated module imports before registration
- Updates skill registry and backlog

Heuristic for "related skills" in backlog:
- Skills in the same domain with names containing the library name
- Skills with names containing the library name (case-insensitive)
"""
...
def get_library_version(library_module: Any) -> str | None:
    ...
    Checks for __version__ attribute first, then falls back to 'version' attribute.
    """
    version = None
...
def is_public_callable(member: Any) -> bool:
    """
    Check if a member is a public callable (function, class, or builtin routine).

    This function handles:
    - Regular functions (inspect.isfunction)
    - Builtin functions and methods (inspect.isroutine)
    - Classes (inspect.isclass)
    - Callable objects that are not frames or other non-api types

    Returns:
        True if the member is a public callable, False otherwise
    """
    ...
    if (
        callable(member)
        and not inspect.isframe(member)
        and not inspect.ismodule(member)
    ):
        if not hasattr(member, "__class__") or member.__class__.__name__ in (
            "property",
            "classmethod",
            "staticmethod",
        ):
...
def get_public_api(library_module: Any) -> List[tuple]:
    ...
    for name, member in inspect.getmembers(library_module):
        ...
        if not is_public_callable(member):
            continue

        public_members.append((name, member))

    logger.info(
        "Found {} public callable members for {}".format(
            len(public_members), module_name
        )
    )
...
'''.format(library_name=library_name, name=name)
    ...
    version_str = library_version if library_version else "unknown"
...
def find_related_skills_in_backlog(library_name: str, domain: str) -> List[Dict]:
    ...
    2. Skills that reference the library name in their name (case-insensitive)

    This heuristic ensures that when a library wrapper is generated,
    any backlog entries that reference that library are marked as implemented.
    ...
    Returns:
        List of related backlog entries that are not yet IMPLEMENTED
    """
    ...
    for entry in backlog:
        ...
        if is_related and entry.get("status") != "IMPLEMENTED":
            ...
            logger.info("Found related backlog entry: {}".format(entry.get("name")))
...
async def invoke(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        skills_root_path = os.path.normpath(os.path.join(skill_path, "..", ".."))

        # Check if a generated file already exists for this library
        generated_file = os.path.join(skill_path, "{}.py".format(library))
        temp_moved_file = None

        # If the generated file exists, temporarily move it to avoid import conflicts
        if os.path.exists(generated_file):
            temp_moved_file = generated_file + ".bak"
            os.rename(generated_file, temp_moved_file)
            logger.info(
                "Temporarily moved generated file: {} -> {}".format(
                    generated_file, temp_moved_file
                )
            )

        try:
            # Now import the real library
            ...
            # Verify we're not using a generated file
            if hasattr(library_module, "__file__") and library_module.__file__:
                module_file = os.path.normpath(library_module.__file__)
                if module_file.startswith(skill_path) or module_file.startswith(
                    skills_root_path
                ):
                    raise Exception("Imported from skill path: {}".format(module_file))

        finally:
            # Restore the generated file if it was moved
            if temp_moved_file and os.path.exists(temp_moved_file):
                os.rename(temp_moved_file, generated_file)
                logger.info(
                    "Restored generated file: {} -> {}".format(
                        temp_moved_file, generated_file
                    )
                )
        ...
        # Restore the file if an error occurred
        if temp_moved_file and os.path.exists(temp_moved_file):
            os.rename(temp_moved_file, generated_file)
        return {
            "result": {"error": "Failed to import library: {}".format(str(e))},
            "metadata": {
                "timestamp": timestamp,
                "error": str(e),
            },
    ...
    if related_skills:
        try:
            ...
            logger.warning("Backlog update failed: {}".format(str(e)))
