...
class DashboardDataCollector:
    ...
    def collect_system_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            max_history = self.config["dashboard"]["history_days"] * 24 * 60 * 60 // self.config["monitoring"]["metrics_interval"]
    ...
    def collect_skill_metrics(self) -> Dict[str, Any]:
        ...
        # This would integrate with the MCP server's telemetry
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        if latest_metrics.get("cpu_usage", 0) > 80:
            ...
            health_status["issues"].append(f"High CPU usage: {latest_metrics['cpu_usage']:.1f}%")
        
        if latest_metrics.get("memory_usage", 0) > 80:
            health_status["status"] = "warning"
            health_status["issues"].append(f"High memory usage: {latest_metrics['memory_usage']:.1f}%")
        
        if latest_metrics.get("disk_usage", 0) > 90:
            health_status["status"] = "warning"
            health_status["issues"].append(f"High disk usage: {latest_metrics['disk_usage']:.1f}%")
...
class MonitoringDashboard:
    ...
    def _setup_routes(self):
        ...
        async def get_metrics_history(hours: int = 24):
            ...
            history = [
                m for m in self.data_collector.metrics_history
                if datetime.fromisoformat(m["timestamp"]) > cutoff_time
            ]
